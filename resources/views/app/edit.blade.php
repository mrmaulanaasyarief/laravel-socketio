            ...
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <div class="max-w-full">
                    <section>
                        ...
                        <form method="post" action="{{ route('center-point.update', $centerPoint->id) }}" class="mt-6 space-y-6">
                            @csrf
                            @method('patch')
                            ...
                            <div class="flex flex-row space-x-4">
                                <div class="basis-1/2">
                                    ...
                                    <x-text-input id="latitude" name="latitude" type="text" class="mt-1 block w-full" :value="old('latitude')"
                                        value="{{ $centerPoint->latitude }}" required readonly/>
                                    <x-input-error class="mt-2" :messages="$errors->get('latitude')" />
                                </div>
                                <div class="basis-1/2">
                                    <x-input-label for="longitude" :value="__('Longitude')" />
                                    <x-text-input id="longitude" name="longitude" type="text" class="mt-1 block w-full" :value="old('logitude')"
                                        value="{{ $centerPoint->longitude }}" required readonly/>
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                @if (session('status') === 'center-point-updated')
                                    <p
                                        x-data="{ show: true }"
                                        x-show="show"
                                        x-transition
